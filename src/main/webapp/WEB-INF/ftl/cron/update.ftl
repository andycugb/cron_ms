    ...
    <table class="table_line">
        <tr>
            ...
            <#if cron.cronName?exists&&cron.cronName!="SYSTEM_CRON_AUTO_REFRESH">
                <input type="text" name="cronName" id="cronName" size="80" value='${cron.cronName?if_exists}'><span
        ...
        <tr>
            ...
            <td>
            <#if cron.cronName?exists&&cron.cronName!="SYSTEM_CRON_AUTO_REFRESH">
                <input type="text" name="serviceName" id="serviceName" size="80"
                       value='${cron.serviceName?if_exists}'><span style='color:red;'>*</span>
            <#else>
            ${cron.serviceName?if_exists}
                <input type="hidden" name="serviceName" id="serviceName" size="80"
                       value='${cron.serviceName?if_exists}'>
            </#if>
            </td>
        </tr>
        <tr>
            <th>执行策略</th>
            <td><input type="text" name="cronExpression" id="cronExpression" size="80"
                       value='${cron.cronExpression?if_exists}'><span style='color:red;'>*</span></td>
        </tr>
        <tr>
            <th>ip限制</th>
            <td><input type="text" name="limitIp" id="limitIp" size="80" value="${cron.limitIp?if_exists}"><span
                    style='color:red;'>*</span>
        ...
        <tr>
            ...
            <#else>
                <#if cron.fireOnStartUp?exists&&cron.fireOnStartUp==0>不执行
                    <input type="hidden" name="fireOnStartUp" id="fireOnStartUp" size="80"
                           value='${cron.fireOnStartUp}'>
                </#if>
                ...
                <select name="groupName" id="groupName">
                    ...
                    <#if cronGroupNames?exists&&cronGroupNames!="">
                        <#list cronGroupNames?split(";") as cronGroupName>
                            ...
                            <option value="${cronGroupName?if_exists}"<#if cron.groupName?exists&&cron.groupName==cronGroupName>
                                    selected</#if>>
