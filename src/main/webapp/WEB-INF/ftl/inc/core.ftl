...
<#macro jsFile file=[] file2=[]>
    ...
    <#list file as x>
    <script src="${contextPath}/${x}?v=${cdnFileVersion}"></script>
    </#list>
</#macro>
<#macro cssFile file=[] file2=[]>
    ...
    <#list file as x>
    <link rel="stylesheet" href="${contextPath}/${x}?v=${cdnFileVersion}"/>
    </#list>
</#macro>
...
<head>
    ...
    <#if !clean>
        <script src="${contextPath}/js/jquery-1.4.2.js?v=201205221605"></script>
...
<#-- 输出一级菜单 -->
    ...
    <ul id="mainMenu">
        <#list menuList as menu>
            <li pid="menu${menu.id}"><a<#if menu.id == curMenuId || _subMenuMatched(menu,curMenuId)>
                    class="active"</#if> href="${menu.url!'/'}"><span>${menu.name}</span></a></li>
        </#list>
    </ul>
    <b class="topShadow"></b>
<#-- 输出二级菜单 -->
    <#list menuList as menu>
        ...
        <#if subMenuList?size gt 0>
            <div id="menu${menu.id}SubMenu"
                 class="topSubMenu<#--<#if curMenuId==menu.id || _subMenuMatched(menu,curMenuId)> fixMenu</#if>-->">
                <ul>
                    <#list subMenuList as subMenu>
                        <li><a<#if curMenuId==subMenu.id> class="active"</#if>
                                                          href="${subMenu.url}"><span>${subMenu.name}</span></a></li>
                    </#list>
                </ul>
            </div>
        </#if>
...
<#macro htmFoot js=[] quickInit=true>
    <#if quickInit>
    <script>Core.quick && Core.quick();Core.quickInit && Core.quickInit();</script>
    </#if>
    <@jsFile file=js />
    <#nested>
...
-->
<#macro _linkRangeForSP a z i url><#list a..z as k><#if i==k><span>${k}</span><#else><a
        href="${url?replace('{p}', k)}">${k}</a></#if></#list></#macro>
<#macro splitPages maxRecordNum=0 totalPage=1 currentPage=1 nearPageNum=3 urlTmpl="?page={p}" wrapCss="splitPages">
    ...
    <div class="${wrapCss}">第<b>${currentPage}</b>页 共<b>${totalPage}</b>页 总<b>${maxRecordNum}</b>条
        ...
        <#elseif (currentPage < nearPageNum + 3)> <#--情况一: 1 2 3 4 5 6 7 8 ... n -->
            <@_linkRangeForSP a=1 z=endMaxLen i=currentPage url=urlTmpl/>
            <em>...</em><@_linkRangeForSP a=totalPage z=totalPage i=currentPage url=urlTmpl/>
        <#elseif (currentPage > totalPage - nearPageNum - 2)> <#-- 情况三: 1 ... 11 12 13 14 15 16 17 18 -->
            <@_linkRangeForSP a=1 z=1 i=currentPage url=urlTmpl/>
            <em>...</em><@_linkRangeForSP a=totalPage-endMaxLen+1 z=totalPage i=currentPage url=urlTmpl/>
        <#else> <#--情况二: 1 ... 3 4 5 * 7 8 9 ... n -->
            <@_linkRangeForSP a=1 z=1 i=currentPage url=urlTmpl/>
            <em>...</em><@_linkRangeForSP a=currentPage-nearPageNum z=currentPage+nearPageNum i=currentPage url=urlTmpl/>
            <em>...</em><@_linkRangeForSP a=totalPage z=totalPage i=currentPage url=urlTmpl/>
        </#if>
